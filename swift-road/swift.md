---
title: "關於 Swift 程式語言開發"
permalink: /swift-road/swift.html
layout: legacy_article
article_section: Swift 殿堂之路
robots: noindex,nofollow
---

# 關於 Swift 程式語言開發

## Swift Bethel of The Road

## 前言

這是這本書的前言這是這本書的前言，也是一個邀請。邀請你進入一個充滿未知與可能性的世界，一個我們將一同探索、一同思考、一同感受的世界。前言，顧名思義，是位於書籍最前端的文字，它像一位熱情的嚮導，在旅程開始之前，先為你勾勒出大致的風景，點亮前方的道路。它並非故事本身，而是故事的引子，是作者與讀者之間建立的第一道橋樑。

### 聲明

這本書的內容是開源的，這意味著它的程式碼、文字、圖像以及其他相關資源都是公開且可自由獲取的。我們相信知識應該共享，並且鼓勵大家學習、使用，甚至基於這些資源進行創作。你可以自由地閱讀、複製、分發這本書的內容，例如，你可以將它分享給你的朋友、同學，或者在你的部落格、網站上引用其中的段落。

### 本書參考自：

* [大話設計模式](http://www.books.com.tw/products/0010430101)
* [Swift设计模式 \(iOS\)](https://www.gitbook.com/book/yourtion/swiftdesignpatterns/details)
* [iOS 中的設計模式 \(Swift版本\) Part 1](http://blog.callmewhy.com/2014/12/29/introducing-ios-design-patterns-in-swift-part-1/)
* [iOS 中的設計模式 \(Swift版本\) Part 2](http://blog.callmewhy.com/2015/03/01/introducing-ios-design-patterns-in-swift-part-2/)

**作者：雄獅資訊 - 互動系統部 - Wilden**

* wildenchen@liontravel.com
* GitBook 編寫

